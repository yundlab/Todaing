...
> **오늘을 한 장으로**  
> 지출·일정·교통·**정산**을 한곳에 모은 개인 가계·라이프 기록 웹앱 (PWA)

---

## 프로젝트 소개

**Todaing**은 하루 단위로 **지출(Expense)** 과 **일정(Schedule)** 을 기록하고, 달력·월별 요약으로 흐름을 보는 **개인용 기록 서비스**입니다. **공동 지출**은 N분의 1로 나누고, **당일 정산표**로 누가 얼마 주고받는지 한 번에 확인할 수 있습니다.
...
- **정산 내역을 헷갈리지 않고 보고 싶을 때** — 같이 쓴 돈·**공동 지출**이 쌓이면 “누가 얼마 냈고, 나는 얼마 부담인지”를 **그날 기준으로** 정리해서 보고 싶다. (채팅·메모에 흩어지지 않게)

👉 Todaing은 **입력 경로를 짧게** 하고, **일정·지출·교통·정산**을 한곳에 모아 **하루 단위로 되돌아보기 쉽게** 만드는 데 초점을 맞췄습니다.
...
- 공동 지출은 **N분의 1**로 나눈 **내 부담액**을 지출 카드·도메인 로직에서 계산
- **정산표** 시트로 해당 일(`dayKey`) 기준 **사람별 주고받을 금액**을 한 화면에서 확인 (누가 받고·누가 낼지 정리)
- 결제 주체(`paymentOwner`)·참여자 명단에 따른 **정산 멤버** 보정(예: 공동인데 명단만 있는 레거시 케이스 등)
...
4. 공동 지출이 있으면 **정산표**로 당일 정산 내역 확인  
5. 일별·월별·달력에서 기록 확인  
