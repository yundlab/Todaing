              ...
              <div className="mt-4 space-y-2">
                {settlementToday.perPerson.size ? (
                  Array.from(settlementToday.perPerson.entries())
                    ...
                    .map(([name, amount]) => {
                      const from = amount >= 0 ? name : settlementToday.me;
                      const to = amount >= 0 ? settlementToday.me : name;
                      const abs = Math.round(Math.abs(amount));
                      const sign = amount >= 0 ? "+" : "-";
                      const settled = isNetSettledForDay(dayKey, name);
                      const amountTone = amount >= 0 ? "text-emerald-700" : "text-rose-700";
                      return (
                        <button
                          key={name}
                          type="button"
                          onClick={() => requestToggleNetSettledForDay(dayKey, name)}
                        ...
                        >
                          ...
                          <div className="flex items-center gap-4">
                            <div
                              className={`flex items-baseline gap-1 tabular-nums ${settled ? "text-slate-400" : amountTone}`}
                            >
                              <span className="text-base font-extrabold tracking-tight">
                                {sign}
                                {abs.toLocaleString()}
